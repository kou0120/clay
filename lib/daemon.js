...
var createWecomBot = require("./wecom-bot").createWecomBot;
...
var ipc = createIPCServer(socketPath(), function (msg) {
  ...
  switch (msg.cmd) {
    ...
    case "external_message": {
      ...
      if (!msg.text || !String(msg.text).trim()) return { ok: false, error: "missing text" };
      var ctx = relay.getProjectContext(msg.slug);
      if (!ctx) return { ok: false, error: "project not found: " + msg.slug };
      var key = msg.sessionKey || ("automation:" + Date.now() + ":" + crypto.randomBytes(4).toString("hex"));
      if (msg.wait) {
        return new Promise(function (resolve) {
          var output = "";
          var settled = false;
          var timeoutMs = msg.timeoutMs || 300000;
          var unsubscribe = ctx.subscribeExternalSession(key, function (event) {
            ...
            if (event.type === "error") {
              ...
              resolve({
                ok: false,
                error: event.text || "Claude returned an error",
                slug: msg.slug,
                sessionKey: key,
              });
              return;
          ...
          var result = ctx.submitExternalMessage(key, String(msg.text), null);
          if (!result || !result.ok) {
            settled = true;
            clearTimeout(timer);
            try { unsubscribe(); } catch (e) { }
            resolve(result || { ok: false, error: "submit failed" });
            return;
          }
      ...
      var result = ctx.submitExternalMessage(key, String(msg.text), null);
      if (!result || !result.ok) return result || { ok: false, error: "submit failed" };
      return {
        ok: true,
        slug: msg.slug,
        sessionKey: key,
...
function startListening() {
  ...
  relay.server.listen(config.port, listenHost, function () {
    ...
    if (config.wecom && config.wecom.enabled) {
      try {
        wecomBot = createWecomBot({
          relay: relay,
          config: config.wecom,
          logger: console,
        });
        if (wecomBot) {
          wecomBot.start();
          console.log("[daemon] WeCom bot enabled for project:", config.wecom.projectSlug);
        }
      } catch (e) {
        console.error("[daemon] Failed to start WeCom bot:", e.message);
      }
    }
  });
}
