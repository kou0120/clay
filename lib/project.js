...
function createProjectContext(opts) {
  ...
  var readonlyBashAllowPatterns = [
    /^pbv\s+/,
    /^cat\s+/,
    /^if\s+\[\s+-s\s+"?\$HOME\/tmp\/suppl\.txt"?\s*\];\s+then\s+echo\s+/,
    /^ls\s+-la\s+\$HOME\/tmp\/suppl\.txt\s+2>\/dev\/null\s+&&\s+cat\s+\$HOME\/tmp\/suppl\.txt\s+2>\/dev\/null\s+\|\|\s+echo\s+/,
    /^test\s+-s\s+\$HOME\/tmp\/suppl\.txt\s+&&\s+cat\s+\$HOME\/tmp\/suppl\.txt\s+\|\|\s+echo\s+/,
    /^echo\s+\$HOME$/,
    /^python3\s+.+\/snakemake_status\.py(\s|$)/,
    /^python3\s+.+\/send_to_qy\.py(\s|$)/,
  ];
  ...
  function submitExternalMessage(key, text) {
    var session = ensureExternalBridgeSession(key);
    var normalized = String(text || "").trim();
    if (!normalized) return { ok: false, error: "Empty message" };
    var userMsg = { type: "user_message", text: normalized };
    session.history.push(userMsg);
    sm.appendToSessionFile(session, userMsg);
    if (!session.title) {
      session.title = normalized.substring(0, 50);
      sm.saveSessionFile(session);
    }
    if (!session.isProcessing) {
      ...
      if (!session.queryInstance && !session.worker) {
        sdk.startQuery(session, normalized, null, getLinuxUserForSession(session));
      } else {
        sdk.pushMessage(session, normalized, null);
      }
    } else {
      sdk.pushMessage(session, normalized, null);
    }
