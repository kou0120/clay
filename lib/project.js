...
function createProjectContext(opts) {
  ...
  // --- Session manager ---
  var externalBridges = {};
  var readonlyToolAllowlist = {
    Read: true,
    Glob: true,
    Grep: true,
    WebFetch: true,
    WebSearch: true,
    "mcp__knowledge-rag__search_knowledge": true,
    "mcp__wecom-doc__create_doc": true,
    "mcp__wecom-doc__edit_doc_content": true,
    "mcp__wecom-doc__get_doc_content": true,
    "mcp__wecom-doc__upload_doc_image": true,
    "mcp__wecom-doc__upload_doc_file": true,
  };
  var readonlyBashDenyPatterns = [
    /\brm\s+/,
    /\brmdir\s+/,
    /\bunlink\s+/,
    ...
    /\btruncate\s+/,
    /\bdd\s+(?:if=|of=)/,
    /^(?![\s\S]*send_to_qy\.py)[\s\S]*\b(?:python|python3|perl|ruby|node|sh|bash|zsh|fish)\b[\s\S]*(?:open\s*\(|writeFile|appendFile|createWriteStream|File\.write|File\.open|(?:^|[^0-9])>\s*["']?)/,
    /\b(?:sed|perl)\b[\s\S]*\s-i(?:\s|$)/,
    /\bawk\b[\s\S]*(?:^|[^0-9])>\s*/,
    /\bxargs\b[\s\S]*\b(?:rm|rmdir|mv|cp|touch|mkdir|chmod|chown|truncate)\b/,
    /\bchmod\s+/,
    /\bchown\s+/,
    /(?:^|[\s;|&])>\s*(?![>&])/,
    /(?:^|[\s;|&])>>\s/,
    /\|\s*tee\s/,
    /\bgit\s+(?:commit|push|add|reset|merge|rebase|stash|checkout|switch|restore|clean|apply|am)\b/,
  ...
  var readonlyBashAllowPatterns = [
    /^\s*(?:pwd|ls|ll|la|tree|find|grep|rg|ag|sed|awk|cat|head|tail|less|more|wc|stat|file|du|df|sort|uniq|cut|tr|basename|dirname|realpath|readlink|which|command\s+-v|type|date|env|printenv|ps|top|free|uname|hostname|id|whoami|groups)(?:\s[\s\S]*)?$/,
    /^\s*git\s+(?:status|log|show|diff|branch|rev-parse|remote|ls-files|grep)(?:\s[\s\S]*)?$/,
    /^\s*snakemake\s+(?:--help|--version|--summary|--detailed-summary|--list|--list-target-rules|--list-params-changes|--list-input-changes|--list-code-changes|--dry-run|-n)(?:\s[\s\S]*)?$/,
    /^\s*python3?\s+(?:"[^"]*send_to_qy\.py"|'[^']*send_to_qy\.py'|\S*send_to_qy\.py)(?:\s[\s\S]*)?$/,
    /^\s*ssh\s+[\w.@-]+\s+(?:"(?:kubectl|oc)\s+(?:get|describe|logs|top|version|api-resources|api-versions|config\s+view)\b[\s\S]*"|'(?:kubectl|oc)\s+(?:get|describe|logs|top|version|api-resources|api-versions|config\s+view)\b[\s\S]*')\s*$/,
  ];
  var readonlySkillAllowlist = {
    "oe-std-status": true,
    "oe-gitlab-repo": true,
  };

  function notifyExternalBridge(session, obj) {
    var bridge = session && session.externalBridgeKey ? externalBridges[session.externalBridgeKey] : null;
    if (!bridge || !bridge.listeners) return;
    var listeners = bridge.listeners.slice();
    for (var i = 0; i < listeners.length; i++) {
      try { listeners[i](obj, session); } catch (e) { }
    }
  }
  ...
  var sm = createSessionManager({
    ...
    onSessionEvent: notifyExternalBridge,
  });
  ...
  var sdk = createSDKBridge({
    ...
    toolPolicy: opts.toolPolicy || null,
  ...
  function ensureExternalBridgeSession(key) {
    var bridge = externalBridges[key];
    if (bridge && bridge.sessionId && sm.sessions.has(bridge.sessionId)) {
      return sm.sessions.get(bridge.sessionId);
    }
    var session = sm.createSessionRaw();
    session.externalBridgeKey = key;
    session.allowedTools = {};
    session.toolPolicy = {
      mode: "readonly",
      allowTools: readonlyToolAllowlist,
      allowSkills: readonlySkillAllowlist,
      allowBashPatterns: readonlyBashAllowPatterns,
      denyBashPatterns: readonlyBashDenyPatterns,
    };
    bridge = bridge || { listeners: [] };
    bridge.sessionId = session.localId;
    externalBridges[key] = bridge;
    return session;
  }

  function submitExternalMessage(key, text, images) {
    var session = ensureExternalBridgeSession(key);
    var normalized = String(text || "").trim();
    var hasImages = images && images.length > 0;
    if (!normalized && !hasImages) return { ok: false, error: "Empty message" };
    var userMsg = { type: "user_message", text: normalized || "(image)" };
    session.history.push(userMsg);
    sm.appendToSessionFile(session, userMsg);
    if (!session.title) {
      session.title = (normalized || "(image)").substring(0, 50);
      sm.saveSessionFile(session);
    }
    var imgs = hasImages ? images : null;
    if (!session.isProcessing) {
      session.isProcessing = true;
      onProcessingChanged();
      session.sentToolResults = {};
      if (!session.queryInstance && !session.worker) {
        sdk.startQuery(session, normalized || "", imgs, getLinuxUserForSession(session));
      } else {
        sdk.pushMessage(session, normalized || "", imgs);
      }
    } else {
      sdk.pushMessage(session, normalized || "", imgs);
    }
    sm.broadcastSessionList();
    return { ok: true, sessionId: session.localId };
  }

  function answerExternalAsk(key, toolId, text) {
    var session = ensureExternalBridgeSession(key);
    ...
    pending.resolve({
      behavior: "allow",
      updatedInput: Object.assign({}, pending.input, { answers: { text: String(text || "") } }),
    });
    return { ok: true, sessionId: session.localId };
  }

  function subscribeExternalSession(key, listener) {
    if (!externalBridges[key]) externalBridges[key] = { listeners: [], sessionId: null };
    externalBridges[key].listeners.push(listener);
    return function unsubscribe() {
      var bridge = externalBridges[key];
      if (!bridge || !bridge.listeners) return;
      bridge.listeners = bridge.listeners.filter(function (fn) { return fn !== listener; });
    };
  }

  function getExternalSessionState(key) {
    var session = ensureExternalBridgeSession(key);
    var pendingQuestions = [];
    var askIds = Object.keys(session.pendingAskUser || {});
    for (var i = 0; i < askIds.length; i++) {
      pendingQuestions.push({ toolId: askIds[i], input: session.pendingAskUser[askIds[i]].input || null });
    }
    return {
      sessionId: session.localId,
      title: session.title || "",
  ...
  var _connection = attachConnection({
    ...
    getOrCreateExternalSession: ensureExternalBridgeSession,
    submitExternalMessage: submitExternalMessage,
    answerExternalAsk: answerExternalAsk,
    subscribeExternalSession: subscribeExternalSession,
    getExternalSessionState: getExternalSessionState,
