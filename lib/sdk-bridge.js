...
function createSDKBridge(opts) {
  ...
  var toolPolicy = opts.toolPolicy || null;
  ...
  function handleCanUseTool(session, toolName, input, opts) {
    var effectiveToolPolicy = session && session.toolPolicy ? session.toolPolicy : toolPolicy;
    if (effectiveToolPolicy && effectiveToolPolicy.mode === "readonly") {
      ...
      if (toolName === "Skill") {
        var skillName = input && input.skill ? String(input.skill).trim() : "";
        if (skillName && effectiveToolPolicy.allowSkills && effectiveToolPolicy.allowSkills[skillName]) {
          return Promise.resolve({ behavior: "allow", updatedInput: input });
        }
      }
      if (toolName === "Bash") {
        var command = input && input.command ? String(input.command).trim() : "";
        var denyBashPatterns = effectiveToolPolicy.denyBashPatterns || [];
        var allowBashPatterns = effectiveToolPolicy.allowBashPatterns || [];
        var bashDenied = false;
        var bashDeniedBy = null;
        for (var i = 0; i < denyBashPatterns.length; i++) {
          if (denyBashPatterns[i] && denyBashPatterns[i].test(command)) {
            bashDenied = true;
            bashDeniedBy = denyBashPatterns[i];
            break;
          }
        }
        var bashAllowed = allowBashPatterns.length === 0;
        for (var j = 0; j < allowBashPatterns.length; j++) {
          if (allowBashPatterns[j] && allowBashPatterns[j].test(command)) {
            bashAllowed = true;
            break;
          }
        }
        if (!bashDenied && bashAllowed) {
          return Promise.resolve({ behavior: "allow", updatedInput: input });
        }
        try {
          console.warn("[sdk-bridge] readonly denied Bash (write op)", {
            command: command,
            pattern: bashDeniedBy ? String(bashDeniedBy) : "not in allowlist",
          });
        } catch (e) { }
        return Promise.resolve({ behavior: "deny", message: "Write operations are not allowed in this chat channel." });
      }
      if (effectiveToolPolicy.allowTools && effectiveToolPolicy.allowTools[toolName]) {
        return Promise.resolve({ behavior: "allow", updatedInput: input });
      }
      try {
        console.warn("[sdk-bridge] readonly denied tool", {
          toolName: toolName,
          input: input,
        });
      } catch (e) { }
      return Promise.resolve({ behavior: "deny", message: "Tool is disabled in this chat channel." });
    }
