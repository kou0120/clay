...
function createSDKBridge(opts) {
  ...
  function getReadonlyBashCandidates(input) {
    var command = input && input.command ? String(input.command).trim() : "";
    if (!command) return [];
    var candidates = [command];
    var wrapped = command.match(/^(?:\/bin\/)?(?:bash|sh)\s+-lc\s+([\s\S]+)$/);
  ...
  function handleCanUseTool(session, toolName, input, opts) {
    ...
    if (effectiveToolPolicy && effectiveToolPolicy.mode === "readonly") {
      ...
      var readonlyBashCommands = [];
      if (toolName === "Bash") {
        var commands = getReadonlyBashCandidates(input);
        readonlyBashCommands = commands;
        var allowBashPatterns = effectiveToolPolicy.allowBashPatterns || [];
        for (var c = 0; c < commands.length; c++) {
          for (var i = 0; i < allowBashPatterns.length; i++) {
            if (allowBashPatterns[i] && allowBashPatterns[i].test(commands[c])) {
              return Promise.resolve({ behavior: "allow", updatedInput: input });
            }
          }
        }
      ...
      try {
        if (toolName === "Bash") {
          console.warn("[sdk-bridge] readonly denied tool", {
            toolName: toolName,
            command: input && input.command ? input.command : "",
            candidates: readonlyBashCommands,
          });
        } else {
          console.warn("[sdk-bridge] readonly denied tool", {
            toolName: toolName,
