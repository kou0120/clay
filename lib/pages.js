...
// --- Shared CSS for multi-user auth pages ---
var authPageStyles =
  '*{margin:0;padding:0;box-sizing:border-box}' +
  'body{background:#2F2E2B;color:#E8E5DE;font-family:system-ui,-apple-system,sans-serif;' +
  'min-height:100dvh;display:flex;align-items:center;justify-content:center;padding:20px}' +
  ...
  'h1{color:#DA7756;font-size:22px;margin-bottom:8px}' +
  '.sub{color:#908B81;font-size:14px;margin-bottom:28px}' +
  '.field{margin-bottom:16px;text-align:left}' +
  '.field label{display:block;font-size:12px;color:#908B81;margin-bottom:6px;text-transform:uppercase;letter-spacing:0.5px}' +
  'input{width:100%;background:#393733;border:1px solid #3E3C37;border-radius:12px;' +
  'color:#E8E5DE;font-size:16px;padding:12px 14px;outline:none;font-family:inherit}' +
  'input:focus{border-color:#DA7756}' +
  'input::placeholder{font-size:14px;color:#6D6860}' +
  '.pin-input{font-size:24px;letter-spacing:12px;text-align:center;-webkit-text-security:disc}' +
  '.btn{width:100%;background:#DA7756;color:#fff;border:none;border-radius:12px;' +
  'padding:14px;font-size:15px;font-weight:600;cursor:pointer;font-family:inherit;' +
  'transition:opacity 0.15s;margin-top:8px}' +
  '.btn:hover{opacity:0.9}' +
  '.btn:disabled{opacity:0.4;cursor:default}' +
  '.err{color:#E5534B;font-size:13px;margin-top:12px;min-height:1.3em}' +
  '.info{color:#908B81;font-size:12px;margin-top:16px}';

// --- Admin Setup Page (multi-user first-time setup) ---
function adminSetupPageHtml() {
  return '<!DOCTYPE html><html lang="en"><head>' +
    ...
    '<h1>Clay</h1>' +
    '<div class="sub">Multi-user setup — Create admin account</div>' +
    '<div class="field"><label>Setup Code</label>' +
    '<input id="code" type="text" maxlength="6" placeholder="6-character code from terminal" autocomplete="off" autofocus></div>' +
    '<div class="field"><label>Username</label>' +
    '<input id="username" type="text" maxlength="30" placeholder="admin" autocomplete="username"></div>' +
    '<div class="field"><label>Display Name</label>' +
    '<input id="displayName" type="text" maxlength="50" placeholder="Admin User"></div>' +
    '<div class="field"><label>PIN</label>' +
    '<input id="pin" class="pin-input" type="tel" maxlength="6" placeholder="6-digit PIN" autocomplete="off" inputmode="numeric"></div>' +
    '<button class="btn" id="submit" onclick="doSetup()" disabled>Create Admin Account</button>' +
    '<div class="err" id="err"></div>' +
    '<div class="info">Enter the setup code shown in the terminal when you ran --multi-user</div>' +
    '<script>' +
    'var codeEl=document.getElementById("code"),userEl=document.getElementById("username"),' +
    'nameEl=document.getElementById("displayName"),pinEl=document.getElementById("pin"),' +
    'btn=document.getElementById("submit"),err=document.getElementById("err");' +
    'function check(){btn.disabled=!(codeEl.value.length>=4&&userEl.value.length>=1&&pinEl.value.length===6)}' +
    'codeEl.addEventListener("input",check);userEl.addEventListener("input",check);' +
    'nameEl.addEventListener("input",check);pinEl.addEventListener("input",check);' +
    'function doSetup(){' +
    'btn.disabled=true;err.textContent="";' +
    'fetch("/auth/setup",{method:"POST",headers:{"Content-Type":"application/json"},' +
    'body:JSON.stringify({setupCode:codeEl.value,username:userEl.value,displayName:nameEl.value||userEl.value,pin:pinEl.value})})' +
    '.then(function(r){return r.json()})' +
    '.then(function(d){' +
    'if(d.ok){location.href="/";return}' +
    'err.textContent=d.error||"Setup failed";btn.disabled=false})' +
...
function multiUserLoginPageHtml() {
  return '<!DOCTYPE html><html lang="en"><head>' +
    ...
    'userEl.addEventListener("input",check);pinEl.addEventListener("input",check);' +
    'pinEl.addEventListener("keydown",function(e){if(e.key==="Enter"&&!btn.disabled)doLogin()});' +
    'function doLogin(){' +
    'btn.disabled=true;err.textContent="";' +
    ...
    '.then(function(d){' +
    'if(d.ok){location.reload();return}' +
    'if(d.locked){pinEl.disabled=true;userEl.disabled=true;' +
    'err.textContent="Too many attempts. Try again in "+Math.ceil(d.retryAfter/60)+" min";' +
    'setTimeout(function(){pinEl.disabled=false;userEl.disabled=false;err.textContent="";userEl.focus()},d.retryAfter*1000);return}' +
    'var msg=d.error||"Invalid credentials";' +
    'if(typeof d.attemptsLeft==="number"&&d.attemptsLeft<=3)msg+=" ("+d.attemptsLeft+" left)";' +
    'err.textContent=msg;pinEl.value="";pinEl.focus();btn.disabled=true})' +
    '.catch(function(){err.textContent="Connection error";btn.disabled=false})}' +
    '</script></div></body></html>';
}
...
function invitePageHtml(inviteCode) {
  return '<!DOCTYPE html><html lang="en"><head>' +
    ...
    '<style>' + authPageStyles + '</style></head><body><div class="c">' +
    '<h1>Clay</h1>' +
    '<div class="sub">You have been invited — Create your account</div>' +
    '<div class="field"><label>Username</label>' +
    '<input id="username" type="text" maxlength="30" placeholder="Choose a username" autocomplete="username" autofocus></div>' +
    '<div class="field"><label>Display Name</label>' +
    '<input id="displayName" type="text" maxlength="50" placeholder="Your name"></div>' +
    '<div class="field"><label>PIN</label>' +
    '<input id="pin" class="pin-input" type="tel" maxlength="6" placeholder="6-digit PIN" autocomplete="off" inputmode="numeric"></div>' +
    '<button class="btn" id="submit" onclick="doRegister()" disabled>Create Account</button>' +
    '<div class="err" id="err"></div>' +
    '<script>' +
    'var inviteCode=' + JSON.stringify(inviteCode) + ';' +
    'var userEl=document.getElementById("username"),nameEl=document.getElementById("displayName"),' +
    'pinEl=document.getElementById("pin"),btn=document.getElementById("submit"),err=document.getElementById("err");' +
    'function check(){btn.disabled=!(userEl.value.length>=1&&pinEl.value.length===6)}' +
    'userEl.addEventListener("input",check);nameEl.addEventListener("input",check);pinEl.addEventListener("input",check);' +
    'function doRegister(){' +
    'btn.disabled=true;err.textContent="";' +
    'fetch("/auth/register",{method:"POST",headers:{"Content-Type":"application/json"},' +
    'body:JSON.stringify({inviteCode:inviteCode,username:userEl.value,displayName:nameEl.value||userEl.value,pin:pinEl.value})})' +
    '.then(function(r){return r.json()})' +
    '.then(function(d){' +
    'if(d.ok){location.href="/";return}' +
    'err.textContent=d.error||"Registration failed";btn.disabled=false})' +
    '.catch(function(){err.textContent="Connection error";btn.disabled=false})}' +
    '</script></div></body></html>';
}

module.exports = { pinPageHtml: pinPageHtml, setupPageHtml: setupPageHtml, adminSetupPageHtml: adminSetupPageHtml, multiUserLoginPageHtml: multiUserLoginPageHtml, invitePageHtml: invitePageHtml };
