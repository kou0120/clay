...
function createWecomBot(opts) {
  ...
  function downloadImage(body) {
    ...
    return wsClient.downloadFile(url, aesKey).then(function (result) {
      ...
      if (fname.match(/\.jpe?g$/i)) mediaType = "image/jpeg";
      else if (fname.match(/\.gif$/i)) mediaType = "image/gif";
      else if (fname.match(/\.webp$/i)) mediaType = "image/webp";
      else if (fname.match(/\.bmp$/i)) mediaType = "image/bmp";
