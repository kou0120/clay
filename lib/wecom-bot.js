...
function createWecomBot(opts) {
  ...
  function subscribeSession(sessionKey) {
    ...
    state.unsubscribe = ctx.subscribeExternalSession(sessionKey, function (event) {
      ...
      if (event.type === "tool_result" && Array.isArray(event.images) && event.images.length > 0) {
        for (var ii = 0; ii < event.images.length; ii++) {
          state.pendingImages.push(event.images[ii]);
        }
        return;
      }
