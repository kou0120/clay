...
function materializeToolServers(sdk, toolServers) {
  if (!toolServers) return toolServers;
  var out = {};
  var names = Object.keys(toolServers);
  for (var i = 0; i < names.length; i++) {
    var name = names[i];
    var server = toolServers[name];
    if (!server || !server.__clayToolServerDef) {
      out[name] = server;
      continue;
    }
    var def = server.__clayToolServerDef;
    var sdkTools = [];
    var tools = def.tools || [];
    ...
    out[name] = sdk.createSdkMcpServer({
      name: def.name,
      version: def.version,
      tools: sdkTools,
    });
...
function createClaudeAdapter(opts) {
  ...
  var adapter = {
    ...
    createQuery: async function(queryOpts) {
      ...
      if (queryOpts.toolServers) sdkOptions.mcpServers = materializeToolServers(sdk, queryOpts.toolServers);
